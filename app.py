...
cred = credentials.Certificate(
    'magia-1c8e4-firebase-adminsdk-fe1mr-4e98a4283d.json')
...
def loading_pdf():
    ...
    blob = bucket.blob(
        'https://storage.googleapis.com/upload/storage/v1/b/magia-1c8e4.appspot.com/o?uploadType=multipart')
    print("_>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")

    with open('/G:/Myfilesmemo/1.pdf', 'rb') as file_obj:
        print("opend")
...
with gr.Blocks(css=css) as demo:
    with gr.Column(elem_id="col-container"):
        ...
        with gr.Column():
            openai_key = gr.Textbox(
                label="You OpenAI API key", type="password")
            pdf_doc = gr.File(label="Load a pdf", file_types=[
                              '.pdf'], type="file")
    ...
    load_pdf.click(pdf_changes, inputs=[pdf_doc, openai_key], outputs=[
                   langchain_status], queue=False)
